...
def test_workflow_service():
    ...
    wf = Workflow(
        name="Test Workflow",
        steps=[
            WorkflowStep(id="s1", prompt_id=p1.id, input_mapping={"val": "input_val"}, output_key="res1"),
            WorkflowStep(id="s2", prompt_id=p2.id, input_mapping={"prev_res": "res1"}, output_key="final")
        ]
    )

    result_context = wf_svc.execute_workflow(wf, {"input_val": "HELLO"})
